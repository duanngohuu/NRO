...
//            e.printStackTrace();
    ...
    public static List<ItemOptionShop> getOption(int itemTemplate) {
        List<ItemOptionShop> list = new ArrayList<>();
        try {
            Connection con = DBService.gI().getConnection();
            PreparedStatement ps = con.prepareStatement("select * from item_option_shop where item_template_id = ?");
            ps.setInt(1, itemTemplate);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new ItemOptionShop(itemTemplate, rs.getInt(3), rs.getInt(4)));
            }
